# Suite de Testes E2E — Post (`/schools/:schoolId/post` e `/post`)
...
| :--- | :--- | :--- |
| Autenticação obrigatória | Rotas de `post` exigem token válido em `Authorization: Bearer <token>`. | Cobrir ausência de token em criação, listagem e atualização, aceitando variação `401`/`498`. |
...
| Criação aninhada em school | `POST /schools/:schoolId/post` obriga passagem de `schoolId` na rota. | Validar que `schoolId` inválido retorna erro; `schoolId` válido cria post com `school_id` correto. |
...
## POST /schools/:schoolId/post — Criação de Post/Comunicado

| Funcionalidade | Comportamento Esperado | Verificações | Critérios de Aceite |
| :--- | :--- | :--- | :--- |
| **Cenários felizes** | | | |
| Cria post com payload válido (scope=all) | Deve persistir post e retornar `201` com todos os metadados. | Enviar `title`, `content`, `target.scope='all'`, `active=true`, `attachments=[]`. | Retorna `201`; `error=false`; `data._id` presente; `data.title` e `data.content` corretos; `data.school_id` igual ao da rota; `data.author_id` igual ao do token; `created_at` e `updated_at` presentes. |
...
| `schoolId` válido mas inexistente | Deve rejeitar por regra de domínio. | Enviar `POST /schools/000000000000000000000000/post` com payload válido. | Retorna `404` ou `422`; mensagem indica "school_id não foi encontrado.". |
...
## GET /post — Listagem de Posts Globais

| Funcionalidade | Comportamento Esperado | Verificações | Critérios de Aceite |
| :--- | :--- | :--- | :--- |
| **Cenários felizes** | | | |
| Admin lista todos os posts | Deve retornar paginação de todos os posts do sistema. | Fazer `GET /post` como admin com token válido. | Retorna `200`; `error=false`; `data.docs` array; `data.totalDocs`, `data.page`, `data.limit` presentes. |
| Paginação padrão | Deve assumir `page=1` e `limit=10`. | Chamar sem query de paginação. | Retorna `200`; metadados condizentes com padrão (`page=1`, `limit=10`). |
| Paginação customizada | Deve respeitar `page` e `limit` enviados. | Chamar `?page=2&limit=5` com massa >5. | Retorna `200`; `page=2`, `limit=5`. |
| Limite máximo de 100 | Deve respeitar limite máximo mesmo se solicitado maior. | Chamar `?limit=150`. | Retorna `200`; `limit=100` na resposta. |
| Filtro por `school_id` | Deve filtrar posts da escola informada. | Criar posts em escolas diferentes, depois consultar `?school_id={schoolA}`. | Retorna `200`; todos os docs contêm `school_id={schoolA}`. |
| Filtro por `author_id` | Deve filtrar posts por autor. | Criar posts com autores diferentes, depois consultar `?author_id={userA}`. | Retorna `200`; todos os docs contêm `author_id={userA}`. |
| Filtro por `title` (parcial/case-insensitive) | Deve filtrar por regex no título. | Consultar `?title=reunião`. | Retorna `200`; apenas docs com "reunião" no título (case-insensitive). |
| Filtro por `content` (parcial/case-insensitive) | Deve filtrar por regex no conteúdo. | Consultar `?content=importante`. | Retorna `200`; apenas docs contendo "importante" no content. |
| Filtro por `active=true` e `active=false` | Deve aplicar filtro de status. | Criar posts ativos e inativos, depois consultar ambas opções. | Retorna `200`; resultado respeita status solicitado. |
| Filtro por `target.scope` | Deve filtrar posts por escopo de target. | Consultar `?scope=class`. | Retorna `200`; apenas docs com `target.scope='class'`. |
| Filtros combinados | Deve respeitar múltiplos filtros simultâneos. | Consultar `?school_id={schoolA}&author_id={userA}&scope=class`. | Retorna `200`; resultado atende a todos os critérios. |
...
| **Cenários tristes** | | | |
| Sem token | Deve bloquear autenticação. | Chamar `GET /post` sem header `Authorization`. | Retorna `498` ou `401`. |
| `school_id` inválido | Deve rejeitar por validação de query. | Chamar `?school_id=abc`. | Retorna `400` ou `422`. |
| `author_id` inválido | Deve rejeitar por validação de query. | Chamar `?author_id=xyz`. | Retorna `400` ou `422`. |
| `active` fora de `true/false` | Deve rejeitar por validação de query. | Chamar `?active=1` ou `?active=maybe`. | Retorna `400` ou `422`. |
...
| :--- | :--- | :--- | :--- |
| **Cenários felizes** | | | |
| Recupera post criado pelo ID | Deve retornar documento completo com metadados. | Fazer `GET /post/{createdPostId}` após criação. | Retorna `200`; `data._id` igual ao criado; `data.title`, `data.content`, `data.author_id`, `data.school_id`, `data.target`, `data.created_at`, `data.updated_at` presentes. |
| **Cenários tristes** | | | |
| Sem token | Deve bloquear autenticação. | Chamar `GET /post/{id}` sem `Authorization`. | Retorna `498` ou `401`. |
| ID com formato inválido | Deve rejeitar por validação de params. | Chamar `GET /post/abc`. | Retorna `400` ou `422`. |
| ID válido mas inexistente | Deve retornar `404`. | Chamar `GET /post/000000000000000000000000`. | Retorna `404`; `error=true`. |
...
| :--- | :--- | :--- | :--- |
| **Cenários felizes** | | | |
| Admin atualiza post de qualquer autor | Deve permitir e persistir mudanças. | Fazer `PATCH /post/{createdPostId}` com `title="Novo título"` como admin. | Retorna `200`; `data.title="Novo título"`; `updated_at` atualizado. |
| Autor não-admin atualiza seu próprio post | Deve permitir. | Fazer `PATCH /post/{createdPostId}` (onde `author_id` é o usuário logado) com `content="Novo conteúdo"`. | Retorna `200`; `data.content="Novo conteúdo"`. |
...
| Admin atualiza `target.scope=all` e limpa `target_id` | Deve remover qualquer `target_id` anterior. | Fazer `PATCH /post/{createdPostId}` que tinha `scope='class'` com novo `target.scope='all'` (sem `target_id`). | Retorna `200`; `target.scope='all'`; `target.target_id` ausente ou nulo. |
| Atualização de `active` | Deve persistir novo status. | Fazer `PATCH /post/{id}` com `active=false`. | Retorna `200`; `data.active=false`. |
| **Cenários tristes** | | | |
| Sem token | Deve bloquear autenticação. | Chamar `PATCH /post/{id}` sem `Authorization`. | Retorna `498` ou `401`. |
| ID com formato inválido | Deve rejeitar por validação de params. | Chamar `PATCH /post/abc`. | Retorna `400` ou `422`. |
| ID válido mas inexistente | Deve retornar `404`. | Chamar `PATCH /post/000000000000000000000000`. | Retorna `404`; `error=true`. |
...
## GET /schools/:schoolId/post — Listagem de Posts da Escola

| Funcionalidade | Comportamento Esperado | Verificações | Critérios de Aceite |
| :--- | :--- | :--- | :--- |
| **Cenários felizes** | | | |
| Admin lista posts da escola | Deve retornar paginação filtrada por `school_id` da rota. | Fazer `GET /schools/{schoolA}/post` como admin. | Retorna `200`; `error=false`; `data.docs` array; todos os docs contêm `school_id={schoolA}`. |
...
| **Cenários tristes** | | | |
| Sem token | Deve bloquear autenticação. | Chamar `GET /schools/{schoolId}/post` sem `Authorization`. | Retorna `498` ou `401`. |
