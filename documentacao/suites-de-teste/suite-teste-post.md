...
| schoolId invalido                      | Deve retornar erro de validacao ou permissao.                                     | POST /schools/invalid-school-id/posts.                                | Retorna 400, 404 ou 422.                                         |
...
| target_id inexistente                  | Deve falhar se o alvo da postagem nao for encontrado.                             | POST com target.scope="class" e target_id aleatorio.                  | Retorna 422 ou 201 (dependendo da massa).                        |
...
| Filtro por titulo                      | Deve filtrar postagens pelo titulo.                              | GET com ?title=Post%20integração.                    | Retorna 200 e docs filtrados.                 |
...
| Id invalido              | Deve rejeitar por validacao de ObjectId.                   | GET /posts/invalid-id-format.                              | Retorna 400 ou 422.                                       |
| Id inexistente           | Deve retornar recurso nao encontrado.                      | GET /posts/507f1f77bcf86cd799439999.                       | Retorna 404 e error=true.                                 |
...
| Id inexistente           | Deve retornar recurso nao encontrado.                      | PATCH /posts/507f1f77bcf86cd799439999.                     | Retorna 404 e error=true.                                 |

## DELETE /posts/:id - Exclusao

| Funcionalidade           | Comportamento Esperado                                     | Verificacoes                                               | Criterios de Aceite                                      |
| :----------------------- | :--------------------------------------------------------- | :--------------------------------------------------------- | :------------------------------------------------------- |
| Exclusao valida          | Deve remover postagem existente.                           | DELETE por postId com token do autor.                      | Retorna 200, error=false, data.message='Anúncio deletado com sucesso'. |
| Id inexistente           | Deve retornar recurso nao encontrado.                      | DELETE /posts/507f1f77bcf86cd799439999.                    | Retorna 404.                                              |
| Post de outro autor      | Deve bloquear delecao se nao for o proprietario.           | DELETE post de autor diferente.                            | Retorna 403 ou 200 (se for admin).                       |
...
| Mensagens de Sucesso         | O DELETE valida explicitamente a string "Anúncio deletado com sucesso".  |
| Ownership                    | A suite valida se a delecao e permitida com base no autor/token.          |
| IDs validos para 404         | Usar 507f1f77bcf86cd799439999 para ObjectId valido inexistente.           |
