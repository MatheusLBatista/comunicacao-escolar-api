...
class ComponenteRepository {
  constructor({ componenteModel = ComponenteModel } = {}) {
  ...
  async criar(parsedData) {
    ...
    return await this.model.findById(componenteSalvo._id).populate('categoria');
  }
  ...
  async atualizar(id, parsedData, req) {
    const componente = await this.model
  ...
  async deletar(id, req) {
    const existeMovimentacao = await MovimentacaoModel.exists({
      componente: id,
    });
    if (existeMovimentacao) {
      throw new CustomError({
        ...
        details: [],
        customMessage:
          'Não é possível deletar: componente está vinculado a movimentações.',
      });
    }
