...
class OrcamentoRepository {
  ...
  async adicionarComponente(orcamentoId, novoComponente, req) {
    ...
    orcamento.total = parseFloat(
      orcamento.componentes
        .reduce((acc, comp) => acc + comp.subtotal, 0)
        .toFixed(2),
    );
  ...
  async atualizarComponente(
    ...
    const componentes = Array.isArray(orcamento.componentes)
      ? orcamento.componentes
      : [];
    const idx = componentes.findIndex(
      (c) => c && c._id && c._id.toString() === componenteId,
    );
    if (idx === -1) {
      throw new CustomError({
        statusCode: 404,
    ...
    componentes[idx] = {
      ...(typeof componentes[idx].toObject === 'function'
        ? componentes[idx].toObject()
        : componentes[idx]),
      ...componenteAtualizado,
    };
    orcamento.componentes = componentes;
    orcamento.total = parseFloat(
      componentes.reduce((acc, comp) => acc + comp.subtotal, 0).toFixed(2),
    );
    await orcamento.save();
  ...
  async removerComponente(orcamentoId, componenteId, req) {
    ...
    orcamento.componentes = orcamento.componentes.filter(
      (c) => c._id.toString() !== componenteId,
    );
    orcamento.total = parseFloat(
      orcamento.componentes
        .reduce((acc, comp) => acc + comp.subtotal, 0)
