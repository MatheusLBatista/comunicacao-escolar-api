...
import {
  ...
  ComponenteOrcamentoSchema,
  ComponenteOrcamentoUpdateSchema,
...
class OrcamentoController {
      ...
      if (!componente) {
        throw new CustomError({
          ...
          details: [
            {
              path: 'componente',
              message: `Componente com ID ${comp.componente} não encontrado.`,
            },
          ],
  ...
  async adicionarComponente(req, res) {
    ...
    if (!componente) {
      throw new CustomError({
        ...
        details: [
          {
            path: 'componente',
            message: `Componente com ID ${parsedComponente.componente} não encontrado.`,
          },
        ],
    ...
    if (!fornecedor) {
      throw new CustomError({
        ...
        details: [
          {
            path: 'fornecedor',
            message: `Fornecedor com ID ${parsedComponente.fornecedor} não encontrado.`,
          },
        ],
    ...
    const orcamentoAtualizado = await this.service.adicionarComponente(
      orcamentoId,
      novoComponente,
      req,
    );
  ...
  async atualizarComponente(req, res) {
    ...
    if (!componenteData || Object.keys(componenteData).length === 0) {
      return CommonResponse.error(res, 400, 'validationError', 'componente', [
        { message: 'Nenhum campo enviado para atualização.' },
      ]);
    }
    const parsedComponente =
      ComponenteOrcamentoUpdateSchema.parse(componenteData);

    // Buscar valores antigos para garantir atualização correta
    const oldComponente = await this.service.getComponenteById(
      orcamentoId,
      id,
      req,
    );
    if (!oldComponente) {
      return CommonResponse.error(res, 404, 'resourceNotFound', 'componente', [
        { message: 'Componente não encontrado.' },
      ]);
    }

    if (
      parsedComponente.fornecedor &&
      parsedComponente.fornecedor !== oldComponente.fornecedor.toString()
    ) {
      const fornecedor = await Fornecedor.findById(parsedComponente.fornecedor);
      if (!fornecedor) {
        throw new CustomError({
    ...
    const orcamentoAtualizado = await this.service.atualizarComponente(
      orcamentoId,
      id,
  ...
  async removerComponente(req, res) {
    const { orcamentoId, id } = req.params;
    const orcamentoAtualizado = await this.service.removerComponente(
      orcamentoId,
      id,
      req,
    );
