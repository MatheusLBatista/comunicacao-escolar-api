...
class ClassService {
  ...
  async create(parsedData, schoolId) {
    ...
    const classExists = await this.repository.existClass(
      schoolId,
      name,
      shift,
      year,
        ...
        details: [
          {
            path: 'name',
            message:
              'Já existe uma turma cadastrada com este nome e série/grau para a escola selecionada.',
          },
        ],
        customMessage:
          'Conflito de registro: Uma turma com este nome e série/grau já existe nesta escola.',
      });
    }

    const profs = await this.userRepository.findUsers(
      parsedData.teacher_ids,
      'teacher',
    );
  ...
  async update(parsedData, schoolId, id) {
    ...
    const classExists = await this.repository.existClass(
      ...
      shift ? shift : turma.shift,
      year ? year : turma.year,
    );
    if (classExists) {
