...
describe('ComponenteQuerySchema', () => {
  ...
  it('deve invalidar nome vazio', () => {
    ...
    expect(() => ComponenteQuerySchema.parse(consulta)).toThrow(
  ...
  it('deve invalidar paginação inválida', () => {
    ...
    expect(() => ComponenteQuerySchema.parse({ page: '0' })).toThrow(
      'Page deve ser um número inteiro maior que 0',
    );
    expect(() => ComponenteQuerySchema.parse({ limite: '101' })).toThrow(
      'Limite deve ser um número inteiro entre 1 e 100',
    );
    expect(() => ComponenteQuerySchema.parse({ limite: '0' })).toThrow(
      'Limite deve ser um número inteiro entre 1 e 100',
    );
  });
