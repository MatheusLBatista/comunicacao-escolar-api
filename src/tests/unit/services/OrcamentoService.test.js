...
describe('OrcamentoService', () => {
  ...
  describe('manipulação de componentes', () => {
    it('deve adicionar componente', async () => {
      const req = { user_id: 'user123' };
      repository.adicionarComponente.mockResolvedValue({
        _id: 'id',
        componentes: [{ nome: 'C1' }],
      });
      const result = await service.adicionarComponente(
        'id',
        { nome: 'C1' },
        req,
      );
      expect(repository.adicionarComponente).toHaveBeenCalledWith(
        'id',
        { nome: 'C1' },
        req,
      );
      expect(result).toEqual(
        expect.objectContaining({ componentes: expect.any(Array) }),
      );
    });
    it('deve atualizar componente', async () => {
      const req = { user_id: 'user123' };
      repository.atualizarComponente.mockResolvedValue({
        _id: 'id',
        componentes: [{ nome: 'C1', quantidade: 2 }],
      });
      const result = await service.atualizarComponente(
        'id',
        'cid',
        { quantidade: 2 },
        req,
      );
      expect(repository.atualizarComponente).toHaveBeenCalledWith(
        'id',
        'cid',
        { quantidade: 2 },
        req,
    ...
    it('deve remover componente', async () => {
      const req = { user_id: 'user123' };
      repository.removerComponente.mockResolvedValue({
        _id: 'id',
        componentes: [],
      });
      const result = await service.removerComponente('id', 'cid', req);
      expect(repository.removerComponente).toHaveBeenCalledWith(
        'id',
        'cid',
        req,
      );
