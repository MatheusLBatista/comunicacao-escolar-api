...
describe('ComponenteService', () => {
  ...
  describe('listar', () => {
    it('deve retornar todos os componentes', async () => {
      const comps = [
        makeComponente(),
        makeComponente({ _id: 'comp2', nome: 'Capacitor' }),
      ];
  ...
  describe('atualizar', () => {
    it('deve atualizar dados do componente, exceto quantidade', async () => {
      ...
      repositoryMock.atualizar.mockResolvedValue(
        makeComponente({ nome: 'Novo Nome', quantidade: 10 }),
      );
    ...
    it('deve lançar erro se tentar atualizar para nome já existente', async () => {
      ...
      repositoryMock.buscarPorNome.mockResolvedValue(
        makeComponente({ _id: 'comp2', nome: 'Duplicado' }),
      );
      await expect(
  ...
  describe('Métodos auxiliares', () => {
    ...
    it('ensureComponentExists lança erro se não existir', async () => {
      repositoryMock.buscarPorId.mockResolvedValue(null);
      await expect(service.ensureComponentExists('compX')).rejects.toThrow(
        CustomError,
      );
    });
    it('ensureComponentExists retorna componente se existir', async () => {
      repositoryMock.buscarPorId.mockResolvedValue(makeComponente());
      await expect(
        service.ensureComponentExists('comp1'),
      ).resolves.toHaveProperty('_id', 'comp1');
    });
